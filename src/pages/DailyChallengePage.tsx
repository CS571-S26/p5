...
function getDailySeed() {
  ...
  const mm = today.getMonth() + 1; // 0-based
...
// NOTE: signature is (seed, n) — keep consistent across calls
...
const DailyChallengePage: React.FC = () => {
  ...
  React.useEffect(() => {
    try {
      ...
      if (!raw) return;
      const parsed: Partial<SavedState> = JSON.parse(raw);
      if (typeof parsed.seed === 'number' && typeof parsed.n === 'number' && Array.isArray(parsed.revealed)) {
        ...
        if (generated.length === parsed.n) {
          setSeed(parsed.seed);
          setN(parsed.n);
          setCurrentIndex(Math.min(parsed.currentIndex ?? 0, (parsed.n ?? 1) - 1));
  ...
  // initialize revealed when puzzle changes (only if sizes mismatch or empty)
  React.useEffect(() => {
    const needsInit =
      !Array.isArray(revealed) ||
      revealed.length !== puzzle.length ||
      revealed.some((r, i) => typeof r !== 'string' || r.length !== puzzle[i].length);

    if (!needsInit) return;

    const initial = puzzle.map((w, idx) =>
